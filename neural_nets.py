...
for i in range(folds):
    ...
    clf = MLPClassifier(activation='logistic',solver='sgd', learning_rate_init=0.001, hidden_layer_sizes=[10], max_iter=10000, batch_size=100)
...
for epoch in range(1,10000):
    clf = MLPClassifier(activation='logistic',solver='sgd', learning_rate_init=0.001, hidden_layer_sizes=[10], max_iter=epoch, batch_size=100)
    clf.fit(X_train,Y_train)
    accuracyvsepoch.append(clf.score(X_test,Y_test))
    
import matplotlib.pyplot as plt

plt.plot(range(1,10000),accuracyvsepoch)
plt.show()
